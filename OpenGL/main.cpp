...
void objectDetection(unsigned& blocks, int& points, int& goldCollected, int& crash)
{
    // collision detection
    int blockCounter = 0;
    int pointCounter = 0;
    for (int i = 0; i < scene.size(); i++)
    {
        ...
        {
            if (CollidesWithBlock(*scene[i]))
            {
                DeleteFromScene(scene[i]->id);
                goldCollected++;
            }
            else if (intersect(garbage2, *scene[i]))
            {
                DeleteFromScene(scene[i]->id);
            }
        }
...
int main()
{
    ...
    double genLocation[7] = { -3.0, -2.0, -1.0, 0.0, 1.0, 2.0, 3.0 };
    ...
    while (!Window::isClosed())
    {
        ...
        if (life > 0)
        {
            ...
            // gamespeed control
            gameSpeed = 0.005 + score * 0.000001;
            ...
            // block and point generations
            if (blocks < 2)
            {
                GameObj3D* block = new GameObj3D(sphereModel);
                block->translate(genLocation[rand() % 7], 0.7, -20.0);
            ...
            if (points < 2)
            {
                ...
                point->translate(genLocation[rand() % 7], 0.7, -20.0);
            ...
            for (int i = 0; i < scene.size(); i++)
            {
                if (scene[i]->type == "fire")
                {
                    scene[i]->moveFront(gameSpeed * 150, true);
                }
                else if (scene[i]->type == "block" || scene[i]->type == "point")
                {
                    scene[i]->moveTowards(garbage.position().x, garbage.position().y, garbage.position().z, gameSpeed);
                }
            }

            auto colliThread = std::thread(objectDetection(), &blocks, &points, &goldCollected, &crash);
